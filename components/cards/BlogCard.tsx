...
export default function BlogCard({ post, body }: Props) {
    ...
    return (
        <Link
            key={post.title}
            href={"blog/" + post.id}
            className={`group rounded-3xl border border-typography/10 bg-background p-6 transition duration-300 hover:-translate-y-1 hover:border-primary/30 flex flex-col gap-4 *:cursor-pointer`}>
